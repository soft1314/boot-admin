...
@Service("Scene2ServiceGeneratorImpl")
public class Scene2ServiceGeneratorImpl implements ServiceGenerator {
    @Autowired
    ...
    public ResultDTO generateFile(String tableName, TgProject tgProject,boolean overwrite) throws Exception {
        ...
        freeMarkerUtils.generate(root, "scene2Service.ftl", path, this.getInterfaceFileName(tableName));
        freeMarkerUtils.generate(root, "scene2ServiceImpl.ftl", path + File.separator + "impl", this.getImplFileName(tableName));
    ...
    public ResultDTO generateCode(String tableName, TgProject tgProject) throws Exception {
        ...
        Map<String, Object> root = new HashMap<>();
        root.put("marker", serviceMarker);
        String srcInterface = freeMarkerUtils.generateSource(root, "scene2Service.ftl");
        String srcImpl = freeMarkerUtils.generateSource(root, "scene2ServiceImpl.ftl");
    ...
    private String getInterfaceClassName(String tableName){
        return "I"+StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene2Service";
    }
    private String getImplClassName(String tableName){
        return StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene2ServiceImpl";
    }
