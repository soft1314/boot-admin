...
@Service("Scene4ViewVueGeneratorImpl")
public class Scene4ViewVueGeneratorImpl implements VueGenerator {
    @Resource
    ...
    public ResultDTO generateCode(String tableName, TgProject tgProject) throws Exception {
        ...
        String source = freeMarkerUtils.generateSource(root, "scene4ViewVue.ftl");
        ...
        sourceCodeVO.setFileName(filePath+File.separator+humpTableName+"Scene4View.vue");
    ...
    public ResultDTO generateFile(String tableName, boolean overwrite ,TgProject tgProject) throws Exception {
        ...
        String humpTableName = StringTool.lineToHump(tableName);
        if(!overwrite) {
            if (isJsFileAlreadyExist(humpTableName+"Scene4View.vue",filePath)) {
        ...
        Map<String, Object> root = new HashMap<>();
        root.put("marker", marker);
        freeMarkerUtils.generate(root, "scene4ViewVue.ftl", filePath, humpTableName+"Scene4View.vue");
    ...
    private String getFilePath(TgProject tgProject,String tableName){
        return tgProject.getViewsDir()  + File.separator + "components" + File.separator +"audit" ;
    }
}
