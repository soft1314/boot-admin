...
@Service("Scene3ServiceGeneratorImpl")
public class Scene3ServiceGeneratorImpl implements ServiceGenerator {
    @Autowired
    ...
    public ResultDTO generateFile(String tableName, TgProject tgProject,boolean overwrite) throws Exception {
        ...
        freeMarkerUtils.generate(root, "scene3Service.ftl", path, this.getInterfaceFileName(tableName));
        freeMarkerUtils.generate(root, "scene3ServiceImpl.ftl", path + File.separator + "impl", this.getImplFileName(tableName));
    ...
    public ResultDTO generateCode(String tableName, TgProject tgProject) throws Exception {
        ...
        Map<String, Object> root = new HashMap<>();
        root.put("marker", serviceMarker);
        String srcInterface = freeMarkerUtils.generateSource(root, "scene3Service.ftl");
        String srcImpl = freeMarkerUtils.generateSource(root, "scene3ServiceImpl.ftl");
    ...
    private String getInterfaceClassName(String tableName){
        return "I"+StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene3Service";
    }
    private String getImplClassName(String tableName){
        return StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene3ServiceImpl";
    }
    ...
    private ResultDTO createMarker(String tableName, TgProject tgProject) throws Exception{
        ...
        /** 列信息 **/
        serviceMarker.setCols(columnService.getTableCols(tableName));
