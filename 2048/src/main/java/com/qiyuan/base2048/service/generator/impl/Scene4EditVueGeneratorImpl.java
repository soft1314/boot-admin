...
@Service("Scene4EditVueGeneratorImpl")
public class Scene4EditVueGeneratorImpl implements VueGenerator {
    ...
    public ResultDTO generateCode(String tableName, TgProject tgProject) throws Exception {
        ...
        String source = freeMarkerUtils.generateSource(root, "scene4EditVue.ftl");
        ...
        sourceCodeVO.setFileName(filePath+File.separator+humpTableName+"Scene4Edit.vue");
    ...
    public ResultDTO generateFile(String tableName, boolean overwrite ,TgProject tgProject) throws Exception {
        ...
        String humpTableName = StringTool.lineToHump(tableName);
        if(!overwrite) {
            if (isJsFileAlreadyExist(humpTableName+"Scene4Edit.vue",filePath)) {
        ...
        Map<String, Object> root = new HashMap<>();
        root.put("marker", marker);
        freeMarkerUtils.generate(root, "scene4EditVue.ftl", filePath, humpTableName+"Scene4Edit.vue");
