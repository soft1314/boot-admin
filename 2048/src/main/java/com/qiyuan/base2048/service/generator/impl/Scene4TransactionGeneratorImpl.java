...
@Service("Scene4TransactionGeneratorImpl")
public class Scene4TransactionGeneratorImpl implements TransactionGenerator {
    @Autowired
    ...
    public ResultDTO generateFile(String tableName, TgProject tgProject,boolean overwrite) throws Exception {
        ...
        freeMarkerUtils.generate(root, "scene4Transaction.ftl", path, this.getInterfaceFileName(tableName));
        freeMarkerUtils.generate(root, "scene4TransactionImpl.ftl", path + File.separator + "impl", this.getImplFileName(tableName));
    ...
    public ResultDTO generateCode(String tableName, TgProject tgProject) throws Exception {
        ...
        Map<String, Object> root = new HashMap<>();
        root.put("marker", serviceMarker);
        String srcInterface = freeMarkerUtils.generateSource(root, "scene4Transaction.ftl");
        String srcImpl = freeMarkerUtils.generateSource(root, "scene4TransactionImpl.ftl");
    ...
    private String getInterfaceClassName(String tableName){
        return "I"+StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene4Transaction";
    }
    private String getImplClassName(String tableName){
        return StringTool.toUpperCaseFirstOne(StringTool.lineToHump(tableName))+"Scene4TransactionImpl";
    }
