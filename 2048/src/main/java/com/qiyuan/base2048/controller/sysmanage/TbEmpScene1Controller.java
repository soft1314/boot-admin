...
@RequestMapping("/api/system/auth/tb-emp/scene1")
@Api("tb_emp表剧情1接口")
public class TbEmpScene1Controller {
  @Resource
  private FormValidator formValidator;
  @Resource
  private ITbEmpScene1Service scene1Service;

  @PostMapping("/table/page")
  @ApiOperation("分页获取项目数据表列表")
  public ResultDTO getTablePage(@Valid @RequestBody TbEmpScene1QueryVO queryVO, BindingResult bindingResult) throws Exception {
  ...
  @PostMapping("/save")
  @ApiOperation("保存数据")
  public ResultDTO save(@Valid @RequestBody TbEmpScene1DataVO dataVO, BindingResult bindingResult) throws Exception {
    if (bindingResult.hasErrors()) {
      return formValidator.generateMessage(bindingResult);
    }
