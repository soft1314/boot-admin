<template>
    <div class="app-container background-white">
                    ...
                    <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
                        ...
                        <#list vue.drawer.formItemList as col>
                            ...
                            <#if col.type == 'SELECT'>
                                <el-form-item label="${col.label}" :label-width="drawerFormLabelWidth"
                                              prop="${col.vmodel}">
                                    <el-select v-model="listQuery.${col.vmodel}" placeholder="${col.placeholder}"
                                               size="mini">
                            ...
                            <#if col.type == 'INPUTWITHBUTTON'>
                                <el-form-item label="${col.label}" :label-width="drawerFormLabelWidth">
                                    <el-input v-model="listQuery.${col.vmodel}" placeholder="${col.placeholder}"
                                              size="mini">
                ...
                <el-table v-loading="loading" :data="tableData" border fit highlight-current-row style="width: 100%">
                    ...
                    <#list vue.elTable.elTableColumnList as col>
                        <el-table-column prop="${ col.prop }" label="${ col.label }"
                                         <#if col.useFormatter>:formatter="${col.prop}ConverterLocal"</#if>></el-table-column>
                    </#list>
        ...
        <#if vue.showAddButton>
            <!-- 对话框区开始 -->
            <el-dialog :visible.sync="addOrEditItemDialogVisible" width="85%">
                <el-form class="demo-form-inline">
                    <#list vue.elDialogForAddOrEditItem.formItemList as col>
                        <#if col.type == 'INPUT'>
                            <el-form-item label="${col.label}" :label-width="addOrEditFormLabelWidth" prop="${col.vmodel}">
                                <el-input placeholder="${col.placeholder}" size="mini" prefix-icon="el-icon-search"
                                          v-model="addOrEditForm.${col.vmodel}">
                                </el-input>
                            </el-form-item>
                        </#if>
                        <#if col.type == 'SELECT'>
                            <el-form-item label="${col.label}" :label-width="addOrEditFormLabelWidth" prop="${col.vmodel}">
                                <el-select v-model="addOrEditForm.${col.vmodel}" placeholder="${col.placeholder}"
                                           size="mini">
                                    <el-option v-for="item in options.${col.vmodel}Options" :key="item.value"
                                               :label="item.label" :value="item.value">
                                    </el-option>
                        ...
                        <#if col.type == 'INPUTWITHBUTTON'>
                            <el-form-item label="${col.label}" :label-width="addOrEditFormLabelWidth">
                                <el-input v-model="addOrEditForm.${col.vmodel}" placeholder="${col.placeholder}"
                                          size="mini">
                                    <el-button slot="append" icon="el-icon-more" @click="${col.click}"></el-button>
                                </el-input>
                            </el-form-item>
        ...
        <el-button @click="saveNewItem">保存</el-button>
        <el-button @click="saveNewItem" type="warning">重置</el-button>
        <el-button @click="addOrEditItemDialogVisible = false">关闭</el-button>
      </span>
            </el-dialog>
            <!-- 对话框区结束 -->
        </#if>
    ...
    export default {
        ...
        data() {
            return {
                ...
                <#if vue.showAddButton>
                addOrEditItemDialogVisible: false,
                addOrEditFormLabelWidth: '200px',
                addOrEditForm: {
                <#list vue.elDialogForAddOrEditItem.formItemList as col>
                ${col.vmodel}: '',
                </#list>
                ...
                options: {
                    tableTypeOptions: [],
                    <#list vue.vueTableColumnDefineDTOList as col>
                    <#if col.optionsName??>
                    ${col.vmodel}Filter:{
                        optionType: '${col.optionsName}'
                    },
                    ${col.vmodel}Options: []
                </#if>
                </#list>
        ...
        mounted() {
            ...
            <#list vue.vueTableColumnDefineDTOList as col>
            <#if col.optionsName??>
            this.${col.vmodel}OptionsFetcher()
            </#if>
            </#list>
        },
        watch: {},
        ...
        methods: {
            ...
            <#if vue.showAddButton>
            addItem() {
                this.addOrEditItemDialogVisible = true
            },
            saveNewItem() {
                alert("todo 在此添加您的代码。")
            },
            ...
            <#list vue.vueTableColumnDefineDTOList as col>
            <#if col.optionsName??>
            async ${col.vmodel}OptionsFetcher() {
                const response = await fetchOptionsByType(this.options.${col.vmodel}Filter)
                this.options.${col.vmodel}Options = response.data
            },
            ${col.vmodel}ConverterLocal(val) {
                return optionValueLabelConverter(this.options.${col.vmodel}Options, val.${col.vmodel})
            },
    </#if>
    </#list>
    <#list vue.vueTableColumnDefineDTOList as col>
