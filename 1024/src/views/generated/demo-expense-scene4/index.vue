            ...
            <div class="demo-drawer__content">
                <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
                         :model="filterDrawer.formData">
                                ...
                                <el-form-item label="标题" :label-width="filterDrawer.formLabelWidth"
                                              prop="title">
                                    <el-input placeholder="请输入标题" size="mini" prefix-icon="el-icon-search"
                                              v-model="filterDrawer.formData.title">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="报销金额" :label-width="filterDrawer.formLabelWidth"
                                              prop="money">
                                    <el-input placeholder="请输入报销金额" size="mini" prefix-icon="el-icon-search"
                                              v-model="filterDrawer.formData.money">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="情况说明" :label-width="filterDrawer.formLabelWidth"
                                              prop="note">
                                    <el-input placeholder="请输入情况说明" size="mini" prefix-icon="el-icon-search"
                                ...
                                <el-form-item label="报销人" :label-width="filterDrawer.formLabelWidth"
                                              prop="empName">
                                    <el-input placeholder="请输入报销人" size="mini" prefix-icon="el-icon-search"
                                              v-model="filterDrawer.formData.empName">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="职称" :label-width="filterDrawer.formLabelWidth" prop="techTitle">
                                    <el-cascader :props="techTitleCascaderProps" ref="techTitleQueryCascaderId" @change="techTitleQueryCascaderChangeEvent" clearable style="width: 100%;">
                                    </el-cascader>
                                    {{filterDrawer.formData.techTitle}}
                                </el-form-item>
                                <el-form-item label="员工级别" :label-width="filterDrawer.formLabelWidth"
                                              prop="empLevel">
                                    <el-select v-model="filterDrawer.formData.empLevel" placeholder="请选择员工级别"
                                               size="mini" clearable>
                                        <el-option v-for="item in optionMap.get($commonDicType.EMP_LEVEL())"
                                                   :key="item.value"
                                                   :label="item.label" :value="item.value"/>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="性别" :label-width="filterDrawer.formLabelWidth"
                                              prop="sex">
                                    <el-select v-model="filterDrawer.formData.sex" placeholder="请选择性别"
                                               size="mini" clearable>
                                        <el-option v-for="item in optionMap.get($commonDicType.SEX())"
                                                   :key="item.value"
                                                   :label="item.label" :value="item.value"/>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="流转状态" :label-width="filterDrawer.formLabelWidth"
                                              prop="flowStatus">
                                    <el-select v-model="filterDrawer.formData.flowStatus" placeholder="请选择流转状态"
                                               size="mini" clearable>
                                        <el-option v-for="item in optionMap.get($commonDicType.FLOW_STATUS())"
                                                   :key="item.value"
                                                   :label="item.label" :value="item.value"/>
                                    </el-select>
                                </el-form-item>
                                <el-form-item label="流程定义ID" :label-width="filterDrawer.formLabelWidth"
                                              prop="processDefinitionId">
                                    <el-input placeholder="请输入流程定义ID" size="mini" prefix-icon="el-icon-search"
                                              v-model="filterDrawer.formData.processDefinitionId">
                                    </el-input>
                                </el-form-item>
                                <el-form-item label="流程实例ID" :label-width="filterDrawer.formLabelWidth"
                                              prop="processInstanceId">
                                    <el-input placeholder="请输入流程实例ID" size="mini" prefix-icon="el-icon-search"
        ...
        <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
            <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%"
                ...
                <el-table-column type="index" label="序号" :index="indexMethod" width="70">
                                ...
                                <el-table-column prop="money" label="报销金额" show-overflow-tooltip sortable></el-table-column>
                                <el-table-column prop="note" label="情况说明" show-overflow-tooltip sortable></el-table-column>
                            <el-table-column prop="bizDate" label="业务日期" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => dateTimeColFormatter(row,column,cellValue)"></el-table-column>
                                <el-table-column prop="empName" label="报销人" show-overflow-tooltip sortable></el-table-column>
                                <el-table-column prop="techTitle" label="职称" show-overflow-tooltip sortable></el-table-column>
                            <el-table-column prop="empLevel" label="员工级别" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.EMP_LEVEL())"></el-table-column>
                            <el-table-column prop="sex" label="性别" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.SEX())"></el-table-column>
                            <el-table-column prop="flowStatus" label="流转状态" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.FLOW_STATUS())"></el-table-column>
        ...
        <el-dialog :visible.sync="mainDataForm.mainDataFormDialogVisible" width="80%" :close-on-click-modal="false"
            ...
            <el-form ref="mainEditForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
                                <el-form-item label="标题" prop="title">
                                    <el-input v-model="mainDataForm.editingRecord.title" placeholder="请输入标题" clearable
                                          :style="{width: '100%'}" />
                                </el-form-item>
                                <el-form-item label="报销金额" prop="money">
                                    <el-input v-model="mainDataForm.editingRecord.money" placeholder="请输入报销金额" clearable
                                          :style="{width: '100%'}" />
                                </el-form-item>
                                <el-form-item label="情况说明" prop="note">
                                    <el-input v-model="mainDataForm.editingRecord.note" placeholder="请输入情况说明" clearable
                                          :style="{width: '100%'}" />
                                </el-form-item>
                            <el-form-item label="业务日期" prop="bizDate">
                                ...
                                <el-form-item label="报销人" prop="empName">
                                    <el-input v-model="mainDataForm.editingRecord.empName" placeholder="请输入报销人" clearable
                                          :style="{width: '100%'}" />
                                </el-form-item>
                          <el-form-item label="职称" prop="techTitle">
                            <el-cascader v-model="cascaderValue.techTitle" :props="techTitleCascaderProps" ref="techTitleEditCascaderId"
                                         style="width: 50%;" @change="techTitleEditCascaderChangeEvent">
                            </el-cascader>
        ...
        methods: {
            ...
            handleCloseMainDataFormDialog() {
                ...
                techTitleLazyLoadData(node, resolve) {
                  ...
                  if(level == 0){
                    const nodes = this.loadLazyCodeNode(this.$commonDicType.TECH_TITLE(),'top',resolve)
                  }else{
                    const {
                        value,
                    } = node;
                    const nodes = this.loadLazyCodeNode(this.$commonDicType.TECH_TITLE(),value,resolve)
                  }
                },
