  ...
  <div class="app-container background-white">
      ...
      <div class="demo-drawer__content">
        <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
          ...
          <el-form-item label="项目名称" :label-width="filterDrawer.formLabelWidth" prop="name">
            <el-input placeholder="请输入项目名称" size="mini" prefix-icon="el-icon-search"
          ...
          <el-form-item label="是否活动" :label-width="filterDrawer.formLabelWidth" prop="defaultProj">
            <el-select v-model="filterDrawer.formData.defaultProj" placeholder="请选择是否活动" size="mini">
              <el-option v-for="item in optionMap.get($commonDicType.YESNO())" :key="item.value"
    ...
    <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%"
        max-height="500" :row-class-name="tableRowClassName">
        ...
        <el-table-column prop="groupId" label="组织(groupId)" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="artifactId" label="项目(artifactId)" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="name" label="项目名称" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="defaultProj" label="活动状态" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.YESNO())"></el-table-column>
        <el-table-column prop="enabled" label="有效状态" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.ENABLED())"></el-table-column>
        <el-table-column prop="enableSwagger" label="启用Swagger" show-overflow-tooltip sortable :formatter="(row,column,cellValue) => colFormatter(row,column,cellValue, $commonDicType.YESNO())"></el-table-column>
        <el-table-column prop="author" label="作者" show-overflow-tooltip sortable></el-table-column>
    ...
    <el-dialog :visible.sync="mainDataForm.mainDataFormDialogVisible" width="80%" :close-on-click-modal="false"
      ...
      <el-form ref="elForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
        <el-form-item label="组织(groupId)" prop="groupId">
          <el-input v-model="mainDataForm.editingRecord.groupId" placeholder="请输入组织(groupId)" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目(artifactId)" prop="artifactId">
          <el-input v-model="mainDataForm.editingRecord.artifactId" placeholder="请输入项目(artifactId)" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目名称" prop="name">
          <el-input v-model="mainDataForm.editingRecord.name" placeholder="请输入项目名称" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="是否启用Swagger" prop="enableSwagger" required>
          <el-switch v-model="mainDataForm.editingRecord.enableSwagger" :active-value="'1'" :inactive-value="'0'"
            active-color="#13ce66" inactive-color="#ff4949" />
        </el-form-item>
        <el-form-item label="代码作者" prop="author">
          <el-input v-model="mainDataForm.editingRecord.author" placeholder="请输入项目代码作者姓名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目包名">
          <el-input v-model="mainDataForm.editingRecord.packageProj" placeholder="请输入项目包的包名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="MP-Entity包名">
          <el-input v-model="mainDataForm.editingRecord.packageEntity" placeholder="请输入项目实体类的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="MP-Mapper包名">
          <el-input v-model="mainDataForm.editingRecord.packageMapper" placeholder="请输入项目Mapper的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="MP-Service包名">
          <el-input v-model="mainDataForm.editingRecord.packageService" placeholder="请输入项目Service的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="MP-Controller包名">
          <el-input v-model="mainDataForm.editingRecord.packageController" placeholder="请输入项目Controller的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="剧情Service包名">
          <el-input v-model="mainDataForm.editingRecord.packageServiceSc" placeholder="请输入项目剧情Service的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="剧情Controller包名">
          <el-input v-model="mainDataForm.editingRecord.packageControllerSc" placeholder="请输入项目剧情Controller的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="VO包名">
          <el-input v-model="mainDataForm.editingRecord.packageVo" placeholder="请输入项目VO的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="Mapstruct包名">
          <el-input v-model="mainDataForm.editingRecord.packageMapstruct" placeholder="请输入项目Mapstruct的包全名" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目JAVA路径">
          <el-input v-model="mainDataForm.editingRecord.javaOutputDir" placeholder="请输入项目JAVA全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目Mapper XML路径">
          <el-input v-model="mainDataForm.editingRecord.xmlOutputDir" placeholder="请输入项目Mapper XML全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目前端VUE路径">
          <el-input v-model="mainDataForm.editingRecord.vueDir" placeholder="请输入前端项目VUE全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目前端API声明JS路径">
          <el-input v-model="mainDataForm.editingRecord.apiJsDir" placeholder="请输入前端项目API声明JS全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="项目前端字典型别JS路径">
          <el-input v-model="mainDataForm.editingRecord.dictypeJsDir" placeholder="请输入前端项目代码类别全局定义JS全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="ChangeLog路径">
          <el-input v-model="mainDataForm.editingRecord.changeLogDir" placeholder="请输入ChangeLog全路径" clearable
            :style="{width: '100%'}" />
        </el-form-item>
      </el-form>
