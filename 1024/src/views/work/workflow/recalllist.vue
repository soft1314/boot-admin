      ...
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%" max-height="500">
        ...
        <el-table-column align="center" label="操作" show-overflow-tooltip min-width="120">
          <template slot-scope="scope">
            <el-button size="least" type="success" @click="handleSeeResource(scope.row,'image')">流图</el-button>
            <el-button size="least" type="success" @click="handleDisplay(scope.row)">跟踪</el-button>
            <el-button size="least" type="primary" @click="handleCompleteTask(scope.row)">撤回</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
<script>
import DynaAuditForm from '../../components/dynaRecallForm'
...
export default {
  ...
  methods: {
    ...
    async getMainTableData() {
      this.loading = false
      const response = await fetchRecallPage(this.filterDrawer.formData)
      this.loading = false
      if (response.code !== 100) {
        this.$message({
          message: response.message,
    ...
    async handleCompleteTask(row) {
      this.$nextTick(() => {
        ...
        this.$refs.dynaAuditFormComponent.setDialogTitle('从【' + row.name + '】撤回')
    ...
    handleDisplay(row) {
      this.$nextTick(() => {
        this.$refs.displayComponent.setSrc('/displayModel.html?processInstanceId=' + row.processInstanceId + '&nocaching=' + new Date().getTime())
