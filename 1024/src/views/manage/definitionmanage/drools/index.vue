  ...
  <div class="app-container background-white">
    ...
    >
      <div class="demo-drawer__content">
        ...
        >
          ...
          <el-form-item label="规则名称" :label-width="filterDrawer.formLabelWidth" prop="name">
            <el-input
          ...
          <el-form-item label="行政区划编码" :label-width="filterDrawer.formLabelWidth" prop="admDivCode">
            <el-input
    ...
    <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%" max-height="500">
          ...
          <template slot-scope="props">
            <el-form label-position="left" class="demo-table-expand">
              ...
              <el-form-item label="行政区划编码">
                <span>{{ props.row.admDivCode }}</span>
              </el-form-item>
        ...
        <el-table-column prop="admDivCode" label="行政区划编码" show-overflow-tooltip sortable />
        <el-table-column
          prop="enabled"
          label="启用状态"
    ...
    >
      <el-form ref="mainEditForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
        <el-form-item label="规则名称" prop="name">
          <el-input v-model="mainDataForm.editingRecord.name" placeholder="请输入规则名称" clearable :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="行政区划编码" prop="admDivCode">
          <el-input
            v-model="mainDataForm.editingRecord.admDivCode"
            placeholder="请输入行政区划编码"
            clearable
            :style="{width: '100%'}"
          />
        ...
        <el-form-item label="规则" prop="rule">
          <el-input v-model="mainDataForm.editingRecord.rule" type="textarea" :rows="50" placeholder="请输入规则" clearable :style="{width: '100%'}" />
        </el-form-item>
...
<script>
import {
  fetchTbDroolsRulePage,
  saveTbDroolsRule,
  delTbDroolsRule
} from '@/api/tb-drools-rule-scene1'
import {
...
export default {
  ...
  methods: {
    ...
    async getMainTableData() {
      this.loading = false
      const response = await fetchTbDroolsRulePage(this.filterDrawer.formData)
      this.loading = false
      if (response.code !== 100) {
        this.$message({
          message: response.message,
    ...
    handleDeleteRow(row) {
      this.$confirm('此操作将删除选中的数据, 是否继续?', '提示', {
        ...
        this.awaitDelTbDroolsRule(row.guid)
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消删除'
    ...
    async awaitDelTbDroolsRule(guid) {
      const guidVO = {
        guid
      }
      const result = await delTbDroolsRule(guidVO)
    ...
    async submitMainDataForm() {
      const response = await saveTbDroolsRule(this.mainDataForm.editingRecord)
      this.$message({
        message: response.message,
        type: 'warning'
      })
