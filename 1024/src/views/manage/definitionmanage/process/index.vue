  ...
  <div class="app-container background-white">
      ...
      <div class="demo-drawer__content">
        <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
          ...
          <el-form-item label="部署编号" :label-width="filterDrawer.formLabelWidth" prop="deploymentId">
            <el-input placeholder="请输入部署编号" size="mini" prefix-icon="el-icon-search"
          ...
          <el-form-item label="类别" :label-width="filterDrawer.formLabelWidth" prop="category">
            <el-input placeholder="请输入类别" size="mini" prefix-icon="el-icon-search"
    ...
    <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%" max-height="500">
        ...
        <el-table-column prop="key" label="标识" show-overflow-tooltip sortable></el-table-column>
        ...
        <el-table-column prop="version" label="版本" show-overflow-tooltip sortable></el-table-column>
    ...
    <el-dialog :visible.sync="mainDataForm.mainDataFormDialogVisible" width="80%" :close-on-click-modal="false"
      ...
      <el-form ref="mainEditForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
        <el-form-item label="类别" prop="processCategory">
          <el-input v-model="mainDataForm.editingRecord.processCategory" placeholder="请输入类别" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-upload class="upload-demo" ref="upload" :action="action" :on-preview="handlePreview"
          :on-remove="handleRemove" :file-list="fileList" :auto-upload="false" multiple :limit="1"
  ...
  export default {
    ...
    methods: {
      ...
      async handleSeeResource(row, resType) {
        ...
        this.listLoading = false;
        if (resType === 'text/xml') {
          let url = this.$commonUtils.genObjectURL(response.data)
