...
<template>
  <div class="app-container background-white">
      ...
      <div class="demo-drawer__content">
        <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
          ...
          <el-form-item label="姓名" :label-width="filterDrawer.formLabelWidth" prop="name">
            <el-input placeholder="请输入姓名" size="mini" prefix-icon="el-icon-search" v-model="filterDrawer.formData.name">
            </el-input>
          </el-form-item>
          <el-form-item label="性别" :label-width="filterDrawer.formLabelWidth" prop="sex">
            <el-select v-model="filterDrawer.formData.sex" placeholder="请选择性别" size="mini" clearable>
              <el-option v-for="item in optionMap.get($commonDicType.SEX())" :key="item.value" :label="item.label"
                :value="item.value" />
            </el-select>
          </el-form-item>
    ...
    <div ref="regionContainer" class="background-gray" style="margin-top: 0px;margin-bottom: 0px;padding-top: 0px;">
      ...
      行政区划:<el-cascader :props="regionCascaderProps" v-model="currentRegion" style="width: 600px;"
        ref="cascaderAdminDivId" @change="regionCascaderChangeEvent"></el-cascader>
      <i class="el-icon-s-home"></i>
      组织机构:<el-cascader :props="organiztionCascaderProps" v-model="currentOrg" style="width: 600px;" ref="cascaderOrgId"
        @change="orgCascaderChangeEvent" :options="orgCascaderOptions"></el-cascader>
    </div>
    <!-- 按钮区域开始 -->
    ...
    <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%" max-height="500">
        ...
        <el-table-column prop="orgName" label="组织机构" show-overflow-tooltip sortable></el-table-column>
        ...
        <el-table-column prop="entryTime" label="入职时间" show-overflow-tooltip sortable
          :formatter="(row,column,cellValue) => dateColFormatter(row,column,cellValue)"></el-table-column>
        <el-table-column prop="birthday" label="出生日期" show-overflow-tooltip sortable
          :formatter="(row,column,cellValue) => dateColFormatter(row,column,cellValue)"></el-table-column>
        <el-table-column prop="job" label="职务" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="adminLevel" label="行政级" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="techLevel" label="技术级" show-overflow-tooltip sortable></el-table-column>
        ...
        <el-table-column prop="remarks" label="备注" show-overflow-tooltip sortable width="50"></el-table-column>
    ...
    <el-dialog :visible.sync="mainDataForm.mainDataFormDialogVisible" width="80%" :close-on-click-modal="false"
      ...
      <el-form ref="mainEditForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
        ...
        <el-form-item label="行政级" prop="adminLevel">
          <el-input v-model="mainDataForm.editingRecord.adminLevel" placeholder="请输入行政级" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="技术级" prop="techLevel">
          <el-input v-model="mainDataForm.editingRecord.techLevel" placeholder="请输入技术级" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        ...
        <el-form-item label="办公电话" prop="telephone">
          <el-input v-model="mainDataForm.editingRecord.telephone" placeholder="请输入办公电话" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="手机号码" prop="mobile">
          <el-input v-model="mainDataForm.editingRecord.mobile" placeholder="请输入手机号码" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="地址" prop="address">
          <el-input v-model="mainDataForm.editingRecord.address" placeholder="请输入地址" clearable
            :style="{width: '100%'}" />
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      jobLazyLoadData(node, resolve) {
        ...
        if (level == 0) {
          const nodes = this.loadLazyCodeNode(this.$commonDicType.TEST(), 'top', resolve)
        } else {
          const {
            value,
          } = node;
          const nodes = this.loadLazyCodeNode(this.$commonDicType.TEST(), value, resolve)
        }
      },
      ...
      regionCascaderChangeEvent(value) {
        ...
        let admDivCode = this.getAdminDivCascaderCurrentValue()
      ...
      lazyLoadRegionData(node, resolve) {
        ...
        if (level == 0) {
          const nodes = this.loadRegionChildrenNode('top', resolve)
        } else {
          const {
            value,
          } = node;
          const nodes = this.loadRegionChildrenNode(value, resolve)
        }
      },
      lazyLoadOrgData(node, resolve) {
        ...
        if (level == 0) {
          const nodes = this.loadOrgChildrenNode('top', admDivCode, resolve)
        } else {
          const {
            value,
          } = node;
          const nodes = this.loadOrgChildrenNode(value, admDivCode, resolve)
        }
      },
