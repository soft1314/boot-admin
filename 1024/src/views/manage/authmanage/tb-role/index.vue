  ...
  <div class="app-container background-white">
      ...
      <div class="demo-drawer__content">
        <el-form class="demo-form-inline" style="margin-top: 25px;margin-right: 20px;" ref="drawerForm"
          ...
          <el-form-item label="主键" :label-width="filterDrawer.formLabelWidth" prop="guid">
            <el-input placeholder="请输入主键" size="mini" prefix-icon="el-icon-search" v-model="filterDrawer.formData.guid">
            </el-input>
          </el-form-item>
          ...
          <el-form-item label="角色编码" :label-width="filterDrawer.formLabelWidth" prop="code">
            <el-input placeholder="请输入角色编码" size="mini" prefix-icon="el-icon-search"
              v-model="filterDrawer.formData.code">
            </el-input>
          </el-form-item>
          <el-form-item label="角色名称" :label-width="filterDrawer.formLabelWidth" prop="name">
            <el-input placeholder="请输入角色名称" size="mini" prefix-icon="el-icon-search"
          ...
          <el-form-item label="启用状态" :label-width="filterDrawer.formLabelWidth" prop="enabled">
            <el-select v-model="filterDrawer.formData.enabled" placeholder="请选择启用状态" size="mini">
          ...
          <el-form-item label="删除状态" :label-width="filterDrawer.formLabelWidth" prop="deleted">
            <el-select v-model="filterDrawer.formData.deleted" placeholder="请选择删除状态" size="mini">
    ...
    <div class="table-container" style="padding: 0;margin: 0px 0px 0px 0px;">
      <el-table v-loading="loading" :data="mainTableData" border fit style="width: 100%" max-height="500">
        ...
        <el-table-column prop="code" label="角色编码" show-overflow-tooltip sortable></el-table-column>
        <el-table-column prop="name" label="角色名称" show-overflow-tooltip sortable></el-table-column>
        ...
        <el-table-column align="center" label="操作" show-overflow-tooltip min-width="180">
          ...
          <el-button type="primary" size="least" @click="editUserPower(scope.row)">用户</el-button>
          <el-button type="primary" size="least" @click="editFunctionPower(scope.row)">功能</el-button>
    ...
    <el-dialog :visible.sync="mainDataForm.mainDataFormDialogVisible" width="80%" :close-on-click-modal="false"
      ...
      <el-form ref="elForm" :model="mainDataForm.editingRecord" :rules="rules" size="medium" label-width="150px">
        <el-form-item label="角色编码" prop="code">
          <el-input v-model="mainDataForm.editingRecord.code" placeholder="请输入角色编码" clearable
            :style="{width: '100%'}" />
        </el-form-item>
        <el-form-item label="角色名称" prop="name">
          <el-input v-model="mainDataForm.editingRecord.name" placeholder="请输入角色名称" clearable
            :style="{width: '100%'}" />
        </el-form-item>
  ...
  export default {
    ...
    methods: {
      ...
      async getMainTableData() {
        this.loading = false
        const response = await fetchTbRolePage(this.filterDrawer.formData)
        this.loading = false
      ...
      async awaitDelTbRole(guid) {
        const guidVO = {
          guid
        }
        const result = await delTbRole(guidVO)
      ...
      async submitMainDataForm() {
        const response = await saveTbRole(this.mainDataForm.editingRecord)
        this.$message({
          message: response.message,
          type: 'warning'
        })
